...
class Game {
    ...
    start () {
        if (!this.started) {
            ...
            document.getElementById("scoreli").innerHTML = `Score: ${this.score}`;          // ALUMNA
            document.getElementById("livesli").innerHTML = `Lives: ${this.player.lives}`;   // ALUMNA
     ...
     */
    removeOpponent () {
        if (this.opponent.speed === 5) {                    // ALUMNA
            document.body.removeChild(this.opponent.image);
            this.opponent = new Boss(this);                 // ALUMNA
        }
        else if (this.opponent.speed === 10) {              // ALUMNA
            document.body.removeChild(this.opponent.image);
            this.endGame(false);                            // ALUMNA
        }
    }
     ...
     */
     endGame (end) {    // ALUMNA
         if (end) {     // ALUMNA
            ...
            let gameWin = new Entity(this, this.width / 2, "auto", this.width / 4, this.height / 4, 0, YOU_WIN_PICTURE)     // ALUMNA
            gameWin.render();   // ALUMNA
        }
    }
